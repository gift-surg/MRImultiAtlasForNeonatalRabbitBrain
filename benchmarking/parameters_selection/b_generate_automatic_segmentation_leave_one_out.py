...
def run_generate_automatic_segmentation_leave_one_out():
    ...
    for target_chart_name in [ph.selected_target]:
        ...
        sub_atlas_charts_with_flipped = np.sort(list(set(atlas_subjects) -
        ...
        spot_sj = SpotDS(atlas_pfo=ph.pfo_benchmarking_parameters_leave_one_out,
                         target_pfo=jph(ph.pfo_benchmarking_parameters_leave_one_out, target_chart_name),
                         target_name=target_chart_name,
                         parameters_tag='CrossValidation')

        # Template parameters:
        spot_sj.atlas_name = 'CrossValidation'  # Multi Atlas Newborn Rabbit
